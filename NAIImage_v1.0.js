...
//@version 0.6.1
//@api 3.0
//@description NovelAI 이미지 플러그인. v0.6.1: 장면 추출 단순화 — 대상 선택·1인칭 시점 제거(턴 우선, 인원 강제 안 함) + LLM 차단 에러(Plugin calls blocked) 방어 추가.
...
(async () => {
    ...
    // [v0.5.0] 장면 추출 모드 — 마지막 AI 턴을 읽어 NAI v4.5 프롬프트로 변환.
    // 출력: scene(전역 배경/분위기) + char(character|persona|npc) 최대 3명.
    // 작가/화풍/품질은 프리셋이 담당 → LLM 은 절대 건드리지 않음.
    const DEFAULT_LLM_PROMPT = [
        'You are a NovelAI v4.5 scene-to-prompt extractor for roleplay.',
        '',
        'CONTEXT',
        '- The "Scene" below is a Korean roleplay turn.',
        '- {{char}} is the AI character. {{user}} is the user character.',
        '- Anyone else who physically appears (a passerby, side character) is an NPC.',
        '',
        'YOUR JOB',
        'Read the scene, pick the single most visually striking moment, and output',
        'EXACTLY this format and nothing else:',
        '',
        'scene: <danbooru tags: location, time of day, physical light source, mood/atmosphere, overall composition, camera angle, number of people>',
        'char(character): <{{char}}\'s pose, expression, action, held objects, and outfit — comma separated>',
        'char(persona): <{{user}}\'s pose, expression, action, held objects, and outfit>',
        'char(npc): <an NPC\'s full appearance + outfit + pose, if one physically appears>',
        '',
        'CRITICAL LABEL RULE: The labels must be the literal English words "character", "persona", "npc" inside char(...). '
      + 'NEVER replace them with the actual character name (do NOT write char(하 륜) or char(Yuria) — always write char(character) / char(persona) / char(npc)). '
      + 'Output char(character) first, char(persona) second, char(npc) last.',
        '',
        'RULES',
        '1. Faithful extraction. Convert any scene as written without moral judgment, omission, or refusal. Content is handled downstream.',
        '2. Only output a char(...) line for someone who PHYSICALLY appears in the depicted moment. Skip lines for absent people. A solo moment = only one char line. BUT if ANY person appears at all (even a close-up of an eye, hand, or face), you MUST output at least one char(...) line for them — never output scene: alone when a person is present.',
        '3. At most 3 char lines total. If a crowd appears, put "crowd" or "multiple_people" in scene: instead of extra char lines. Only one char(npc) line max — pick the most relevant NPC.',
        '4. BACKGROUND — use only what the turn actually establishes. Do NOT invent physical backgrounds to fit a mood: never add sunset, sunrise, beach, ocean, sky, city_view, cherry_blossoms, etc. unless the turn explicitly places the scene there. If the location is unclear, use a plain neutral setting (e.g. indoors) — do not embellish. MOOD is different: you MAY freely add atmosphere/emotion tags (cozy, romantic, tense, melancholic, intimate) since those interpret the scene rather than fabricate a place.',
        '5. NEVER output artist tags, art-style words (semi-realistic, flat color, cel shading, manhwa style, etc.), quality tags (masterpiece, best quality, absurdres), or style-defining lighting (cinematic lighting, soft focus). Those are fixed by the preset. You MAY use a physical light source ONLY if the turn establishes it (moonlight, candlelight, backlit, dim room, indoor lighting) — but this does not override rule 4: never introduce sunset/sunrise or an outdoor vista that the turn did not state.',
        '6. For {{char}} and {{user}}, their fixed appearance (hair, eyes, body) may be supplied separately — still, you MAY include appearance if helpful; duplicates are harmless. For NPCs, always describe full appearance since they have no sheet.',
        '7. Use English Danbooru-style tags with underscores for compound terms (looking_at_viewer, holding_cup, school_uniform).',
        '8. Ignore inner thoughts, dialogue text, and time-passing narration unless physically depicted.',
        '9. Output English only, even if the scene is Korean.',
        '{targetDirective}',
        '',
        'Scene:',
        '{scene}'
    ...
    // System 메시지 디폴트 (1인 모드에서도 동일하게 사용 가능 — 거부 회피용)
    ...
    // Assistant prefill — 1인 모드는 단일 라인 출력이라 prefill 없는 게 자연스러움.
    // (장면 추출 모드 부활 시 '[Scene]\n' 으로 바꿔야 함)
    const DEFAULT_ASSISTANT_PREFILL = '';

    // ── 1인 모드 — 포즈/표정 옵션 (모드 탭에서 버튼 형태로 노출) ────
    // 각 카테고리에서 0~여러 개 토글 가능. id 가 config.profileOverride.options 에 저장됨.
    const POSE_PRESETS = [
        { id: 'smile',         label: '😊 미소',     tags: 'smile, looking_at_viewer' },
        { id: 'serious',       label: '😐 무표정',   tags: 'expressionless, looking_at_viewer' },
        { id: 'shy',           label: '😳 수줍음',   tags: 'blush, looking_down, embarrassed' },
        { id: 'playful',       label: '😜 장난',     tags: 'grin, playful' },
        { id: 'cool',          label: '😎 시크',     tags: 'serious, looking_to_the_side' },
        { id: 'sad',           label: '🥺 슬픔',     tags: 'sad, downcast_eyes' },
        { id: 'angry',         label: '😠 화남',     tags: 'angry, frown' },
        { id: 'closeup',       label: '🔍 클로즈업', tags: 'face_focus, portrait, close-up' },
        { id: 'upper_body',    label: '👤 상반신',   tags: 'upper_body' },
        { id: 'full_body',     label: '🧍 전신',     tags: 'full_body, standing' },
        { id: 'from_side',     label: '↔️ 측면',     tags: 'from_side' },
        { id: 'from_above',    label: '⬆️ 위에서',   tags: 'from_above' }
    ];
    ...
    const DEFAULT_CONFIG = {
        ...
        llmFallback: true,
        // [v0.5.1] LLM 프롬프트를 작업별 2슬롯으로 분리
        //   sheetPrompt : 1인 프로필 모드 — 캐릭터 시트 → 한 줄 태그
        //   scenePrompt : 장면 추출 모드 — 마지막 턴 → scene/char(role)
        sheetPrompt: DEFAULT_LLM_PROMPT_SHEET,
        scenePrompt: DEFAULT_LLM_PROMPT,
        // NSFW 우회 / 거부 회피
        llmUseSystem: true,
        llmUsePrefill: false,        // 프로바이더가 assistant prefix 거부하면 자동 폴백됨
        ...
        // 1인 모드 — 포즈/표정 오버라이드
        profileOverride: {
            useDefault: true,         // LLM이 시트에서 추출한 디폴트 포즈/표정 사용
            options: [],              // POSE_PRESETS 의 id 배열
            extra: ''                 // 자유 입력 태그
        },
        // 참조 이미지 (UI만, 백엔드 준비 중 — v0.4.1 Character Reference)
        refImages: { char: '', persona: '', style: '' },
        // [v0.5.0] 장면 추출 모드 옵션
        sceneTarget: 'auto',     // 'auto' | 'p1' | 'p2' | 'p3'
        scenePov: false,         // 1인칭 시점 (고른 대상의 시점)
        sceneSheetMap: [-1, -1, -1],  // 인물 1·2·3 슬롯별 외형 시트 인덱스 (-1 = 사용 안 함)
        // 마이그레이션 플래그
    ...
    async function loadConfig() {
        try {
            ...
            // [v0.5.0] 모드 — profile | free | scene 셋 다 유효
            if (!['profile', 'free', 'scene'].includes(config.generateMode)) {
                config.generateMode = 'profile';
            }
            // [v0.5.0] 장면 추출 옵션 검증
            // [v0.6.0] 대상 값 마이그레이션 char→p1, user→p2
            if (config.sceneTarget === 'char') config.sceneTarget = 'p1';
            else if (config.sceneTarget === 'user') config.sceneTarget = 'p2';
            if (!['auto', 'p1', 'p2', 'p3'].includes(config.sceneTarget)) config.sceneTarget = 'auto';
            if (typeof config.scenePov !== 'boolean') config.scenePov = false;
            if (!Array.isArray(config.sceneSheetMap) || config.sceneSheetMap.length !== 3) {
                config.sceneSheetMap = [-1, -1, -1];
            }
            config.sceneSheetMap = config.sceneSheetMap.map(v => {
                const n = parseInt(v, 10);
                return (Number.isFinite(n) && n >= -1) ? n : -1;
            });
            delete config.sceneUseSheet;
            ...
            // 오버라이드
            if (!config.profileOverride || typeof config.profileOverride !== 'object') {
                config.profileOverride = { useDefault: true, options: [], extra: '' };
            } else {
                config.profileOverride.useDefault = config.profileOverride.useDefault !== false;
                if (!Array.isArray(config.profileOverride.options)) config.profileOverride.options = [];
                if (typeof config.profileOverride.extra !== 'string')  config.profileOverride.extra = '';
            }
            // [v0.5.1] LLM 프롬프트 2슬롯 마이그레이션
            //   구버전 llmPresets 배열 / 단일 llmPrompt 가 있으면 시트·장면 슬롯으로 이관.
            if (typeof config.sheetPrompt !== 'string' || !config.sheetPrompt.trim()) {
                let sheetBody = DEFAULT_LLM_PROMPT_SHEET;
                let sceneBody = DEFAULT_LLM_PROMPT;
                const arr = Array.isArray(saved.llmPresets) ? saved.llmPresets : [];
                for (const p of arr) {
                    const body = (p && typeof p.body === 'string') ? p.body : '';
                    if (!body.trim()) continue;
                    // {sheet} 를 쓰면 시트용, {scene} 나 char(...) 형식이면 장면용으로 추정
                    if (/\{sheet\}/i.test(body)) sheetBody = body;
                    else if (/\{scene\}/i.test(body) || /char\s*\(/i.test(body) || /\[\s*scene\s*\]/i.test(body)) sceneBody = body;
                }
                // 단일 legacy llmPrompt (아주 구버전) 도 흡수
                if (typeof saved.llmPrompt === 'string' && saved.llmPrompt.trim()) {
                    if (/\{sheet\}/i.test(saved.llmPrompt)) sheetBody = saved.llmPrompt;
                    else sceneBody = saved.llmPrompt;
                }
                config.sheetPrompt = sheetBody;
                config.scenePrompt = sceneBody;
            }
            if (typeof config.scenePrompt !== 'string' || !config.scenePrompt.trim()) {
                config.scenePrompt = DEFAULT_LLM_PROMPT;
            }
            // 구조 정리 — 더 이상 안 쓰는 배열 필드 제거
            delete config.llmPresets;
            delete config.activeLlmPreset;
            delete config.llmPrompt;
            delete config.profileMigrationDone;

            // NSFW 우회 옵션 안전 기본값
            if (typeof config.llmUseSystem !== 'boolean')  config.llmUseSystem  = true;
            if (typeof config.llmUsePrefill !== 'boolean') config.llmUsePrefill = true;
    ...
    function looksLikeRefusal(text) {
        const t = (text || '').trim();
        if (!t) return true;
        // API 에러 문자열이 응답으로 온 경우 — 프롬프트에 새면 안 됨
        if (/invalid_request_error|wrong_api_format|is\s+unsupported|"code"\s*:\s*"[^"]*error|요청\s*에러|validation_error|plugin\s+calls?\s+are\s+blocked|blocked\s+by\s+the\s+caller|call(?:s|ing)?\s+(?:is|are)\s+blocked/i.test(t)) return true;
        if (t.length < 30) return true; // 너무 짧으면 거부 가능성
        // 정답 패턴이 있으면 거부 아님
        if (/^\s*scene\s*[:：]/im.test(t) || /char\s*\(\s*(?:character|persona|npc)\s*\)/i.test(t)) return false;
    ...
    // forceNoPrefill=true 면 프리필을 강제로 빼고 구성 (프로바이더가 prefix 거부 시 폴백용)
    function buildLlmMessages(userPrompt, forceNoPrefill) {
        const msgs = [];
        if (config.llmUseSystem && config.llmSystemPrompt) {
            msgs.push({ role: 'system', content: config.llmSystemPrompt });
        }
        msgs.push({ role: 'user', content: userPrompt });
        if (!forceNoPrefill && config.llmUsePrefill && config.llmAssistantPrefill) {
    ...
    async function refineScene(sceneText, signal, targetDirective) {
        if (config.llmMode === 'off') return null;
        if (typeof risuai.runLLMModel !== 'function') {
            console.warn('[NAI Image] runLLMModel 미지원 → 턴 그대로 사용');
            return null;
        }
        // [v0.5.1] 장면 추출 전용 프롬프트 슬롯 사용
        const promptBody = (typeof config.scenePrompt === 'string' && config.scenePrompt.trim())
            ? config.scenePrompt : DEFAULT_LLM_PROMPT;
        // {{char}} {{user}} 먼저 치환, 그 다음 {scene}/{targetDirective} 치환
        const resolved = await resolveRisuVars(promptBody);
        const prompt = resolved
            .replace(/\{targetDirective\}/g, targetDirective || '')
            .replace(/\{scene\}/g, sceneText);

        // prefill 을 썼다면 응답 앞에 그 prefill 을 붙여서 파싱해야 일관됨
        const prefillEcho = config.llmUsePrefill ? config.llmAssistantPrefill : '';

        // 프로바이더가 assistant prefix 를 거부하면(no-prefill) 자동 재시도하기 위해
        // 프리필 on/off 두 버전의 메시지를 준비.
        const tryOnce = async (mode, noPrefill) => {
            const messages = buildLlmMessages(prompt, noPrefill);
            ...
            out = out.toString();
            // 프로바이더가 prefix 미지원 에러를 내면 프리필 빼고 한 번 더
            if (!noPrefill && config.llmUsePrefill
                && /prefix.*unsupported|wrong_api_format|assistant.*prefix/i.test(out)) {
                console.warn('[NAI Image] assistant prefill 미지원 → 프리필 없이 재시도');
                return await tryOnce(mode, true);
            }
            // 응답에 prefill 이 안 붙어 왔다면 우리가 붙여서 파싱 (프리필을 실제로 쓴 경우만)
            if (!noPrefill && prefillEcho && !/^\s*scene\s*[:：]/im.test(out)) out = prefillEcho + out;
            return out;
        };
        ...
        let out1 = '';
        try {
            out1 = await tryOnce(config.llmMode === 'sub' ? 'submodel' : 'model', false);
            console.log('[NAI Image] LLM 원본 응답 ↓\n' + out1);
            if (!looksLikeRefusal(out1)) {
                const parts = parseLLMParts(out1);
                console.log('[NAI Image] 파싱 결과: scene=' + (parts.scene ? 'O' : 'X')
                    + ', chars=' + (parts.chars ? parts.chars.length : 0)
                    + (parts.chars && parts.chars.length ? ' [' + parts.chars.map(c => c.role).join(', ') + ']' : ''));
                if (parts.scene || (parts.chars && parts.chars.length)) return parts;
            }
        ...
        if (config.llmMode === 'sub' && config.llmFallback) {
            try {
                const out2 = await tryOnce('model', false);
                if (!looksLikeRefusal(out2)) {
                    const parts2 = parseLLMParts(out2);
                    if (parts2.scene || (parts2.chars && parts2.chars.length)) return parts2;
                }
    ...
    async function analyzeSheet(sheetBody, signal) {
        ...
        // [v0.5.1] 시트 분석 전용 프롬프트 슬롯 사용
        const promptBody = (typeof config.sheetPrompt === 'string' && config.sheetPrompt.trim())
            ? config.sheetPrompt : DEFAULT_LLM_PROMPT_SHEET;
        const resolved = await resolveRisuVars(promptBody);
        ...
        const tryOnce = async (mode, noPrefill) => {
            ...
            if (!noPrefill && config.llmUsePrefill
                && /prefix.*unsupported|wrong_api_format|assistant.*prefix/i.test(out)) {
                console.warn('[NAI Image] (시트) assistant prefill 미지원 → 프리필 없이 재시도');
                return await tryOnce(mode, true);
            }
            return out;
        };
    ...
    // [v0.5.0] 새 형식 파서:  scene: ...  / char(character): ... / char(persona): ... / char(npc): ...
    function parseLLMParts(text) {
        ...
        const rawChars = [];   // 순서 보존: { explicitRole|null, action }
        for (const rawLine of lines) {
            ...
            let m = line.match(/^(?:scene|setting|background)\s*[:：]\s*(.+)$/i);
            if (m) { scene = (scene ? scene + ', ' : '') + m[1].trim(); continue; }

            // char(...): ...   — 괄호 안이 뭐든( character/persona/npc/이름/char1 ) 다 수용
            //   [role]: ... 이나 role: ... 형태도 허용
            m = line.match(/^char\s*\(\s*([^)]*?)\s*\)\s*[:：]\s*(.+)$/i)
              || line.match(/^\[\s*(character|persona|npc)\s*\]\s*[:：]\s*(.+)$/i)
              || line.match(/^(character|persona|npc)\s*[:：]\s*(.+)$/i);
            if (m) {
                const rawLabel = (m[1] || '').trim().toLowerCase();
                const body = (m[2] || '').trim();
                if (!body || /^<.*>$/.test(body) || body.toLowerCase() === 'none' || body === '-' || body === 'n/a') continue;

                // 라벨에서 역할 힌트 추출 (영단어면 명시, 이름이면 null → 순서로 결정)
                let explicitRole = null;
                if (/\b(character|char\b|char1)\b/.test(rawLabel)) explicitRole = 'character';
                else if (/\b(persona|user|char2)\b/.test(rawLabel)) explicitRole = 'persona';
                else if (/\bnpc\b/.test(rawLabel)) explicitRole = 'npc';
                rawChars.push({ explicitRole, action: body });
            }
        }

        // 역할 배정: 명시된 건 그대로, 이름 라벨은 남은 슬롯을 순서대로(character→persona→npc)
        const chars = [];
        const usedRole = { character: false, persona: false, npc: false };
        // 1차: 명시적 역할 먼저 확정
        for (const rc of rawChars) {
            if (rc.explicitRole && !usedRole[rc.explicitRole]) {
                usedRole[rc.explicitRole] = true;
                rc._assigned = rc.explicitRole;
            }
        }
        // 2차: 미배정(이름 라벨/중복)을 남은 슬롯에 순서대로
        const order = ['character', 'persona', 'npc'];
        for (const rc of rawChars) {
            if (rc._assigned) { chars.push({ role: rc._assigned, action: rc.action, outfit: '' }); continue; }
            const free = order.find(r => !usedRole[r]);
            if (!free) continue; // 3명 초과분 버림
            usedRole[free] = true;
            chars.push({ role: free, action: rc.action, outfit: '' });
        }

        // 아무것도 못 찾으면 전체를 scene 으로 폴백
        if (!scene && !chars.length && t.trim()) {
            return { scene: t.trim(), chars: [], npc: '' };
        }
    ...
    // [v0.5.6] 장면 추출 — 인물 1·2·3 슬롯에 지정된 시트의 추출 태그를 순서대로 반환.
    // config.sceneSheetMap[n] = profileSheets 인덱스 (-1 = 사용 안 함)
    // 반환: ['시트태그' | '', '', ''] — chars 배열의 n번째 인물 캡션 앞에 붙는다.
    function resolveSceneSlotTags() {
        const sheets = config.profileSheets || [];
        const map = Array.isArray(config.sceneSheetMap) ? config.sceneSheetMap : [-1, -1, -1];
        return [0, 1, 2].map(i => {
            const idx = map[i];
            if (idx == null || idx < 0 || idx >= sheets.length) return '';
            return (sheets[idx].extractedTags || '').trim();
        });
    }
    ...
    function buildPrompt(rawScene, parts, opts) {
        ...
        // [v0.5.0] 장면 추출 모드 — parts 가 들어오면 scene(전역) + char(개별 상자)
        const hasLLM = parts && (parts.scene || (parts.chars && parts.chars.length));
        if (!hasLLM) {
            // LLM 실패/off — 정제된 원문 턴을 그대로 scene 으로
        ...
        // [v0.5.6] 인물 슬롯 태그 (opts.slotTags = ['시트태그1','시트태그2','시트태그3'])
        const slotTags = (opts && opts.slotTags) || ['', '', ''];
        const povTag = (opts && opts.pov) ? 'pov' : '';

        // 인물별 캡션 조립 — n번째 인물 앞에 n번 슬롯 태그
        const charList = (parts.chars || []);
        const charCaptions = [];
        for (let i = 0; i < charList.length; i++) {
            const c = charList[i];
            const fixedAppear = trim(slotTags[i] || '');
            const segs = [
                fixedAppear,        // 슬롯 고정 외형 (시트 추출+편집 태그)
                trim(c.action),     // LLM 추출: 포즈/표정/복장/소품
                povTag              // POV 시점 태그 (선택)
            ].filter(Boolean);
        ...
        // 인물이 1명이면 char_captions 안 쓰고 base 에 합침 (NAI 가 더 안정적)
        if (charCaptions.length <= 1) {
            const solo = charCaptions.length === 1 ? charCaptions[0].caption : '';
            const baseSegs = [
                trim(artist),
                trim(parts.scene),
                trim(solo),
        ...
        // 2~3명 — base(전역) + char_captions(개별 격리)
        const baseSegs = [
            trim(artist),
            trim(parts.scene),
            trim(positive)
    ...
    async function handleGenerate(opts) {
        ...
        try {
            ...
            if (mode === 'free') {
                ...
                // 포즈/표정 오버라이드 결합
                const ov = config.profileOverride || { useDefault: true, options: [], extra: '' };
                const optionTags = (ov.options || [])
                    .map(id => POSE_PRESETS.find(p => p.id === id)?.tags)
                    .filter(Boolean)
                    .join(', ');
                // useDefault=true 면 LLM 추출 디폴트 사용 + 오버라이드 더하기
                // useDefault=false 면 외형 부분만 남기고 LLM 의 표정/포즈 부분은 사용자 오버라이드로 대체
                // (지금 단계에선 추출 태그가 한 줄이라 외형/포즈 분리 어려움 → 일단 둘 다 더해주는 방식)
                const allTags = [
                    sheet.extractedTags,
                    optionTags,
                    (ov.extra || '').trim()
                ...
                promptObj = buildPrompt(null, null, { profileTags: allTags });
                job.phase = 'generate'; await saveJob(job); renderWindow();
            } else if (mode === 'scene') {
                // [v0.5.0] 장면 추출 모드 — 마지막 AI 턴 읽어서 LLM 변환
                job.phase = 'extract'; await saveJob(job); renderWindow();
                ...
                // [v0.5.6] 인물 슬롯별 시트 태그 결합
                const slotTags = resolveSceneSlotTags();

                job.phase = 'generate'; await saveJob(job); renderWindow();
                promptObj = buildPrompt(cleaned, parts, {
                    slotTags
                });
            } else {
    ...
    async function openWindow() {
        ...
        document.body.innerHTML = `
          <style>
            ...
            .gi-subtabs { display:flex; gap:6px; margin-bottom:14px; }
            .gi-subtab { flex:1; text-align:center; padding:9px 0; font-size:12.5px; font-weight:600; cursor:pointer;
                         border-radius:9px; background:#fbf7f8; border:1px solid #efe5e6; color:#b8a0a0; user-select:none; }
            .gi-subtab.active { background:#fbedf0; border-color:#f0c8d2; color:#e07a8f; }
            ...
            /* 1인 모드 — 포즈/표정 옵션 버튼 그리드 */
            .gi-posegrid { display:grid; grid-template-columns:repeat(3, 1fr); gap:6px; }
            .gi-poseopt { padding:8px 4px; border:1px solid #efe5e6; background:#fbf7f8; color:#78646a;
                          border-radius:8px; font-size:11.5px; cursor:pointer; text-align:center; font-weight:500;
                          font-family:inherit; }
            .gi-poseopt:hover { background:#f6ebed; }
            .gi-poseopt.active { background:#fdeef2; border-color:#e07a8f; color:#e07a8f; font-weight:600; }
            ...
            .gi-checkbox-row { display:flex; align-items:center; gap:7px; }
            .gi-checkbox-row input[type=checkbox] { width:14px; height:14px; flex:0 0 14px; }
            .gi-checkbox-row label { font-size:11.5px; font-weight:500; cursor:pointer; line-height:1.4; }
            .gi-slotrow { display:flex; align-items:center; gap:8px; margin-top:6px; }
            .gi-slotlabel { font-size:11.5px; font-weight:500; flex:0 0 auto; min-width:80px; color:#8a7378; }
            .gi-slotrow select {
                flex:1; background:#fbf7f8; border:1px solid #efe5e6; border-radius:8px;
                color:#685458; padding:8px 10px; font-size:12px; font-family:inherit; box-sizing:border-box;
            }
          ...
          <div class="gi-win" id="gi-win">
            ...
            <div class="gi-tabs">
              ...
              <div class="gi-tab" id="gi-tab-manage">관리</div>
        ...
        document.getElementById('gi-tab-manage').addEventListener('click', () => { activeView = 'manage'; renderWindow(); });
    ...
    function syncTabs() {
        ...
        document.getElementById('gi-tab-manage')?.classList.toggle('active', activeView === 'manage');
        const isSettings = ['settings','llm_edit','history','refimages'].includes(activeView);
    ...
    function renderWindow() {
        ...
        else if (activeView === 'manage')   { renderManage(screen); stopElapsed(); }
    ...
    // ── 관리 탭 (시트 / 프리셋) ──────────────────────────────────────
    let manageTab = 'sheet';  // 'sheet' | 'preset'
    function renderManage(screen) {
        screen.innerHTML = `
          <div class="gi-subtabs">
            <div class="gi-subtab ${manageTab === 'sheet' ? 'active' : ''}" data-mtab="sheet">캐릭터 시트</div>
            <div class="gi-subtab ${manageTab === 'preset' ? 'active' : ''}" data-mtab="preset">프리셋</div>
          </div>
          <div id="gi-manage-body"></div>
        `;
        screen.querySelectorAll('.gi-subtab').forEach(t => {
            t.addEventListener('click', () => {
                manageTab = t.getAttribute('data-mtab');
                renderManage(screen);
            });
        });
        const body = screen.querySelector('#gi-manage-body');
        if (manageTab === 'sheet') renderSheetManage(body);
        else renderPresetManage(body);
    }
    ...
    function renderMode(screen) {
        ...
        const activeLlmName = (m === 'scene') ? '장면 추출' : '캐릭터 시트 분석';

        const sheets = config.profileSheets || [];
        const sheetIdx = (config.activeProfileSheet < sheets.length) ? config.activeProfileSheet : 0;
        const sheet = sheets[sheetIdx];
        const sheetOptsNone = ['<option value="-1">사용 안 함</option>']
            .concat(sheets.map((s, i) => `<option value="${i}">${esc(s.name)}</option>`)).join('');
        // 프사 모드 시트 드롭다운 (사용 안 함 없음)
        const sheetOpts = sheets.length
            ? sheets.map((s, i) => `<option value="${i}" ${i === sheetIdx ? 'selected' : ''}>${esc(s.name)}</option>`).join('')
            : '<option value="-1">시트 없음 — 관리 탭에서 추가</option>';
        const cacheStatus = !sheet ? ''
            : !sheet.extractedTags ? '<span style="color:#c0a8a8;">⚠️ 아직 분석 안 됨 — 생성 시 자동 분석돼요</span>'
            : (sheet.sheetUpdatedAt && sheet.lastAnalyzedAt && sheet.sheetUpdatedAt > sheet.lastAnalyzedAt)
                                    ? '<span style="color:#b8862a;">⚠️ 시트가 수정됨 — 관리 탭에서 다시 분석 권장</span>'
            : `<span style="color:#2f9e6e;">✓ 캐시됨 (${fmtTime(sheet.lastAnalyzedAt)})</span>`;

        // 장면 슬롯 드롭다운 헬퍼
        const slotSelect = (i) => {
            const sel = (config.sceneSheetMap && config.sceneSheetMap[i] != null) ? config.sceneSheetMap[i] : -1;
            const opts = ['<option value="-1"' + (sel === -1 ? ' selected' : '') + '>사용 안 함</option>']
                .concat(sheets.map((s, si) => `<option value="${si}" ${sel === si ? 'selected' : ''}>${esc(s.name)}</option>`)).join('');
            return `<select class="gi-scene-slot" data-slot="${i}">${opts}</select>`;
        };
        ...
        screen.innerHTML = `
          <div class="gi-section">
            ...
            <div class="gi-presetnow">현재: ${esc(active.name)}${active.bindAi ? ' · AI설정 묶임' : ''} · <span style="color:#c0a8a8;">편집은 관리 탭</span></div>
          </div>

          <div class="gi-section">
            <h3>생성 방식</h3>
            <div class="gi-radiorow">
              <div class="gi-radio ${m === 'profile' ? 'active' : ''}" data-mode="profile">프사</div>
              <div class="gi-radio ${m === 'scene' ? 'active' : ''}" data-mode="scene">장면</div>
              <div class="gi-radio ${m === 'free' ? 'active' : ''}" data-mode="free">자유</div>
            </div>
          </div>
          ...
          <div class="gi-section">
            ...
            <div class="gi-presetnow">${cacheStatus} · <span style="color:#c0a8a8;">편집은 관리 탭</span></div>
          </div>
          ` : m === 'scene' ? `
          <div class="gi-section">
            <h3>장면 추출</h3>
            <div style="margin-top:2px;">
              <div class="gi-slotrow" style="margin-top:0;"><span class="gi-slotlabel">인물 1 외형</span>${slotSelect(0)}</div>
              <div class="gi-slotrow"><span class="gi-slotlabel">인물 2 외형</span>${slotSelect(1)}</div>
              <div class="gi-slotrow"><span class="gi-slotlabel">인물 3 외형</span>${slotSelect(2)}</div>
            </div>
            <div class="gi-freenote">마지막 AI 턴을 그대로 읽어 장면을 만들어요. 턴에 나온 인물만 그려지고, 인원을 억지로 채우지 않아요. 시트를 걸면 그 인물(등장 순서 1·2·3) 외형이 고정돼요 — 안 걸면 LLM이 알아서 뽑아요. 시트는 <b style="color:#e07a8f;">관리 탭</b>에서 만들어요.${config.llmMode === 'off' ? '<br><b style="color:#c8526c;">⚠️ LLM이 꺼져 있어요. 설정에서 켜야 장면 분석이 됩니다.</b>' : ''}</div>
          </div>
          ` : `
          <div class="gi-section">
            <h3>자유 태그</h3>
          ...
          <div class="gi-section" style="text-align:center;">
            ...
            ${(m === 'profile' || m === 'scene') && config.llmMode !== 'off' ? `<div class="gi-estimate" style="margin-top:3px;">LLM 프롬프트: <b style="color:#e07a8f;">${esc(activeLlmName)}</b></div>` : ''}
          </div>
        ...
        // 모드 토글
        screen.querySelectorAll('.gi-radio[data-mode]').forEach((r) => {
        ...
        document.getElementById('gi-sheet-select')?.addEventListener('change', async (e) => {
            ...
            if (v >= 0) { config.activeProfileSheet = v; await saveConfig(); renderMode(screen); }
        });

        // 장면 모드 — 인물 슬롯만
        screen.querySelectorAll('.gi-scene-slot').forEach((sel) => {
            sel.addEventListener('change', async (e) => {
                const slot = parseInt(e.target.getAttribute('data-slot'), 10);
                const val = parseInt(e.target.value, 10);
                if (!Array.isArray(config.sceneSheetMap)) config.sceneSheetMap = [-1, -1, -1];
                if (slot >= 0 && slot < 3) config.sceneSheetMap[slot] = Number.isFinite(val) ? val : -1;
                await saveConfig();
            });
        });
        ...
        document.getElementById('gi-go')?.addEventListener('click', async () => {
            const ta = document.getElementById('gi-free-tags');
            if (ta) { config.freeTags = ta.value; await saveConfig(); }
            handleGenerate();
        });
    ...
    function renderSettings(screen) {
        ...
        screen.innerHTML = `
          ...
          <div class="gi-subpages" style="margin:0;">
            <div class="gi-subrow" data-go="manage">
              <span>📝 프리셋 · 시트 관리</span><span class="gi-chevr">›</span>
            </div>
          </div>
    ...
    // ── 프리셋 편집 하위 페이지 ─────────────────────────────────────
    function renderPresetManage(screen) {
        const active = config.presets[config.activePreset] || config.presets[0];
        const presetOpts = config.presets.map((p, i) =>
            `<option value="${i}" ${i === config.activePreset ? 'selected' : ''}>${esc(p.name)}</option>`).join('');
    ...
    function renderLlmEdit(screen) {
        ...
        screen.innerHTML = `
          ...
          <div class="gi-pagetitle">LLM 설정
            <span class="sub">1인 프로필은 시트를, 장면 추출은 마지막 턴을 태그로 변환합니다. 자유 태그 모드에서는 사용 안 함.</span>
          </div>
          ...
          <div class="gi-field">
            ...
            <span class="hint">거부 키워드나 API 형식 에러가 섞인 응답은 거부로 간주해 안전하게 폴백합니다.</span>
          </div>
          ...
          <div class="gi-field gi-field-new">
            ...
            <span class="hint">일부 프로바이더(Gemini 계열 등)는 prefill을 거부합니다. 그럴 땐 자동으로 프리필 없이 재시도해요.</span>
          </div>

          <div class="gi-field">
            <label>① 캐릭터 시트 분석 프롬프트 <span style="color:#c0a8a8;font-weight:400;">(1인 프로필 모드)</span></label>
            <textarea id="gi-sheet-prompt" style="min-height:200px; font-family:ui-monospace,Menlo,Consolas,monospace; font-size:12px;">${esc(config.sheetPrompt)}</textarea>
            <span class="hint"><b style="color:#e07a8f;">{sheet}</b> 자리에 캐릭터 시트가 들어갑니다. 결과는 한 줄 Danbooru 태그.</span>
          </div>

          <div class="gi-field">
            <label>② 장면 추출 프롬프트 <span style="color:#c0a8a8;font-weight:400;">(장면 추출 모드)</span></label>
            <textarea id="gi-scene-prompt" style="min-height:240px; font-family:ui-monospace,Menlo,Consolas,monospace; font-size:12px;">${esc(config.scenePrompt)}</textarea>
            <span class="hint"><b style="color:#e07a8f;">{scene}</b> 자리에 마지막 턴, <b style="color:#e07a8f;">{targetDirective}</b> 자리에 대상/POV 지시가 자동 삽입됩니다. 결과는 <b style="color:#e07a8f;">scene:</b> / <b style="color:#e07a8f;">char(...)</b> 형식.</span>
          </div>
          ...
          <div class="gi-field">
            ...
            <span class="hint">"System 메시지 사용" 켜져있을 때만 적용됩니다. (시트·장면 공통)</span>
          </div>

          <div class="gi-field">
            <label>Assistant Prefill (옵션)</label>
            <input id="gi-llm-prefill" type="text" value="${esc(config.llmAssistantPrefill)}" />
            <span class="hint">"Assistant prefill 사용" 켜져있을 때만. 프로바이더가 거부하면 자동으로 빼고 재시도합니다.</span>
          </div>

          <button class="gi-save" id="gi-llm-save">저장</button>
          <div class="gi-saved" id="gi-llm-saved"></div>
          <div style="padding:0 16px 14px;">
            <button class="gi-reset" id="gi-llm-reset">⚠️ 두 프롬프트 모두 기본값으로 되돌리기</button>
          </div>
          <div style="height:14px;"></div>
        `;
        ...
        document.getElementById('gi-llm-save')?.addEventListener('click', async () => {
            ...
            config.sheetPrompt = document.getElementById('gi-sheet-prompt').value || DEFAULT_LLM_PROMPT_SHEET;
            config.scenePrompt = document.getElementById('gi-scene-prompt').value || DEFAULT_LLM_PROMPT;
        ...
        document.getElementById('gi-llm-reset')?.addEventListener('click', async () => {
            if (!confirm('시트·장면 프롬프트를 모두 기본값으로 되돌릴까요?')) return;
            config.sheetPrompt = DEFAULT_LLM_PROMPT_SHEET;
            config.scenePrompt = DEFAULT_LLM_PROMPT;
            config.llmSystemPrompt = DEFAULT_SYSTEM_PROMPT;
            config.llmAssistantPrefill = DEFAULT_ASSISTANT_PREFILL;
    ...
    // ── [v0.4.0] 캐릭터 시트 편집 하위 페이지 ───────────────────────
    function renderSheetManage(screen) {
        const sheets = config.profileSheets || [];
        const idx = config.activeProfileSheet < sheets.length ? config.activeProfileSheet : 0;
        const active = sheets[idx];
        const sheetOpts = sheets.map((s, i) =>
            `<option value="${i}" ${i === idx ? 'selected' : ''}>${esc(s.name)}</option>`).join('');
        const stale = active && (!active.extractedTags
                              || !active.lastAnalyzedAt
                              || (active.sheetUpdatedAt && active.sheetUpdatedAt > active.lastAnalyzedAt));
        const cacheStatus = !active ? ''
            : !active.extractedTags ? '⚠️ 아직 분석 안 됨'
            : stale                  ? '⚠️ 시트가 수정됨 — 다시 분석 권장'
            : `✓ 분석됨 (${fmtTime(active.lastAnalyzedAt)})`;

        screen.innerHTML = `
          <div class="gi-pagetitle" style="padding-top:4px;">캐릭터 시트
            <span class="sub">캐릭터의 외모/성격을 적으면 LLM 이 Danbooru 태그로 분석합니다. 시트 본문이 바뀌면 캐시가 stale 표시되고 다음 생성 시 자동 재분석 돼요.</span>
          </div>

          <div class="gi-field">
            <label>시트 선택</label>
            <div class="gi-presetrow">
              <select id="gi-se-select">${sheetOpts}</select>
              <button class="gi-iconbtn" id="gi-se-new"  title="새 시트">＋</button>
              <button class="gi-iconbtn" id="gi-se-copy" title="현재 시트 복사">📋</button>
              <button class="gi-iconbtn" id="gi-se-del"  title="삭제">🗑</button>
            </div>
            <button class="gi-btn ghost" id="gi-se-autofetch" style="width:100%; margin-top:8px;">📥 현재 대화 캐릭터 빼오기</button>
            <span class="hint">지금 대화 중인 캐릭터의 설정(이름·외모·성격)을 시트 본문으로 자동으로 불러와요.</span>
          </div>

          <div class="gi-field">
            <label>이름</label>
            <input id="gi-se-name" type="text" value="${esc(active?.name || '')}" />
          </div>

          <div class="gi-field">
            <label>시트 본문</label>
            <textarea id="gi-se-body" style="min-height:220px; font-family:inherit; font-size:12.5px; line-height:1.6;">${esc(active?.body || '')}</textarea>
            <span class="hint">예시 항목: 이름 / 나이 / 외모(머리·눈·체형) / 복장 / 성격 / 특이사항. 항목 이름은 자유롭게 정해도 LLM 이 알아서 해석합니다.</span>
          </div>

          <div class="gi-field gi-field-new">
            <label>추출된 태그 (편집 가능)</label>
            <textarea id="gi-se-tags" style="min-height:80px; font-family:ui-monospace,Menlo,Consolas,monospace; font-size:11.5px; line-height:1.6;">${esc(active?.extractedTags || '')}</textarea>
            <div class="gi-presetnow" style="font-size:11px;">${cacheStatus}</div>
            <div class="gi-btnrow" style="margin-top:8px;">
              <button class="gi-btn ghost" id="gi-se-analyze">▶ 다시 분석</button>
              <button class="gi-btn ghost" id="gi-se-clear">태그 비우기</button>
            </div>
            <span class="hint">분석 결과가 마음에 안 들면 직접 편집하세요. 저장하면 캐시로 보존됩니다.</span>
          </div>

          <button class="gi-save" id="gi-se-save">저장</button>
          <div class="gi-saved" id="gi-se-saved"></div>
          <div style="height:14px;"></div>
        `;

        // 현재 대화 캐릭터 빼오기 (Auto 기능)
        document.getElementById('gi-se-autofetch')?.addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            const orig = btn.textContent;
            btn.disabled = true; btn.textContent = '불러오는 중…';
            try {
                if (typeof risuai.getCurrentCharacterIndex !== 'function' || typeof risuai.getCharacterFromIndex !== 'function') {
                    throw new Error('이 RisuAI 버전은 캐릭터 빼오기를 지원하지 않아요.');
                }
                const ci = await risuai.getCurrentCharacterIndex();
                const ch = await risuai.getCharacterFromIndex(ci);
                if (!ch) throw new Error('현재 대화 중인 캐릭터 정보를 찾을 수 없습니다.');
                const s = config.profileSheets[config.activeProfileSheet];
                if (!s) return;
                const name = ch.name || ch.title || '';
                const data = ch.data || ch;
                let desc = data.desc || data.description || '';
                if (desc.length > 8000) desc = desc.substring(0, 8000) + '...(이하 생략)';
                let persona = data.personality || '';
                if (persona.length > 4000) persona = persona.substring(0, 4000) + '...(이하 생략)';
                let newBody = `[이름]: ${name}`;
                if (desc) newBody += `\n\n[외모/설명]:\n${desc}`;
                if (persona) newBody += `\n\n[성격]:\n${persona}`;
                s.body = newBody;
                s.name = name ? name + ' (자동)' : '자동 생성된 시트';
                s.sheetUpdatedAt = Date.now();
                await saveConfig();
                renderSheetManage(screen);
            } catch (err) {
                alert((err && err.message) || '캐릭터 정보를 불러오지 못했습니다.');
            } finally {
        ...
        const capture = () => {
            const a = config.profileSheets[config.activeProfileSheet];
            if (!a) return;
            const oldBody = a.body;
            a.name = (document.getElementById('gi-se-name')?.value || a.name).trim() || a.name;
            a.body = document.getElementById('gi-se-body')?.value ?? a.body;
            a.extractedTags = (document.getElementById('gi-se-tags')?.value || '').trim();
            if (a.body !== oldBody) a.sheetUpdatedAt = Date.now();
        };

        document.getElementById('gi-se-select')?.addEventListener('change', (e) => {
            capture();
            config.activeProfileSheet = parseInt(e.target.value, 10) || 0;
            renderSheetManage(screen);
        });
        document.getElementById('gi-se-new')?.addEventListener('click', () => {
            capture();
        ...
        document.getElementById('gi-se-copy')?.addEventListener('click', () => {
            capture();
            const cur = config.profileSheets[config.activeProfileSheet];
            config.profileSheets.splice(config.activeProfileSheet + 1, 0, {
                name: (cur.name || '시트') + ' (복사)',
                body: cur.body,
                extractedTags: cur.extractedTags,
                lastAnalyzedAt: cur.lastAnalyzedAt,
                sheetUpdatedAt: cur.sheetUpdatedAt
            });
            config.activeProfileSheet += 1;
            renderSheetManage(screen);
        });
        document.getElementById('gi-se-del')?.addEventListener('click', () => {
        ...
        document.getElementById('gi-se-analyze')?.addEventListener('click', async (e) => {
            capture();
            const btn = e.currentTarget;
            const s = config.profileSheets[config.activeProfileSheet];
        ...
        document.getElementById('gi-se-clear')?.addEventListener('click', () => {
            if (!confirm('현재 시트의 추출된 태그를 비울까요?')) return;
            const a = config.profileSheets[config.activeProfileSheet];
            if (a) { a.extractedTags = ''; a.lastAnalyzedAt = 0; }
            renderSheetManage(screen);
        });
        document.getElementById('gi-se-save')?.addEventListener('click', async () => {
            capture();
            await saveConfig();
            const s = document.getElementById('gi-se-saved');
            if (s) { s.textContent = '저장됨'; setTimeout(() => (s.textContent = ''), 1500); }
        });
    }
    ...
    async function renderHistory(screen) {
        ...
        const rows = list.map((e, idx) => {
            ...
            const modeTxt = e.modeTag === 'free'  ? '<span class="gi-hmode">· 자유</span>'
                          : e.modeTag === 'scene' ? '<span class="gi-hmode">· 장면</span>'
                          : e.freeMode            ? '<span class="gi-hmode">· 자유</span>'
                          : '';
    ...
    await risuai.registerButton(
        { name: 'NAI Image', icon: '🎨', iconType: 'html', location: 'chat', id: 'btn-naiimage' },
        () => { activeView = 'main'; openWindow(); }
    );
    await risuai.registerSetting?.('NAI Image', () => { activeView = 'settings'; openWindow(); }, '🎨', 'html');
    await risuai.onUnload?.(async () => { stopElapsed(); await risuai.hideContainer?.(); });

    console.log('[NAI Image v0.6.1] loaded');
})();
